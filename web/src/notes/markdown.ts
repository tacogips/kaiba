...
// inline grammar (bold, italic, inline code, links). Parsing lives here, free of
// DOM and framework imports, so the reader, the table of contents and the tests
// all read the same document model.
...
/** Inline markup stripped down to its visible text, used for heading slugs and
 * for anywhere a heading is shown outside the rendered body. */
export function plainInlineText(text: string): string {
  return parseInlineSegments(text).map((segment) => segment.text).join('')
}
