...
swift run kaiba --version | tail -n 1 | grep -Fx "$version"
```
...
```text
dist/homebrew-cask/kaiba-<version>-darwin-arm64.dmg
dist/homebrew-cask/kaiba-<version>-darwin-x64.dmg
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/kaiba-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/kaiba-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/kaiba-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/kaiba-${version}-darwin-x64.dmg"
```
...
`../homebrew-tap/Casks/kaiba.rb`.

After reviewing the rendered tap Cask:

```bash
cd ../homebrew-tap
git add Casks/kaiba.rb README.md
git diff --staged --stat
git commit -m "chore: release kaiba <version>"
...
```bash
brew fetch --cask user/tap/kaiba
HOMEBREW_NO_GITHUB_API=1 brew audit --cask user/tap/kaiba
```
...
After pushing `Casks/kaiba.rb`, require `tacogips/homebrew-tap`'s
`update-api-metadata.yml` workflow to succeed for that commit. Verify the
GitHub Raw `api/cask/kaiba.json`: its `.version` must equal the release version
and its `.ruby_source_checksum.sha256` must equal the SHA-256 of the committed
Cask. Do not consider the release complete while the endpoint is missing or
stale.
